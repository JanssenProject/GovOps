...
A core objective is to define how organizations and open source projects use formally specified policy languages (with well-defined semantics) to reason about authorization behavior for well-defined analysis tasks—e.g., safety over a bounded entity set—not only for individual access requests, so that downstream consumers can verify or reason about the authorization behavior of components they adopt. Where policy is machine-readable and designed to support composition across the supply chain (with composition semantics to be specified or scoped as future work), governance scales across projects and vendors. For policies and mechanisms that are not machine-analyzable, the WG will define how to scope and document review procedures so governance remains consistent. 
